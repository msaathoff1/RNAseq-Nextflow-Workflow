...
parser = argparse.ArgumentParser(description="This will use a pre-specified query to examine a database and return "
                                             "information")
parser.add_argument('fastqcFile', help='The output fastqc path containing the fastqc_data.txt file that needs parsing')
parser.add_argument('fqName', help='The input.zip file minus the .zip suffix. ie name.zip becomes name')
args = parser.parse_args()

def get_phred(fqfile, fqname):
    with ZipFile(fqfile, "r") as zipObj:
        zipObj.extractall()
                    ...
                    if s1 != -1:
                        ...
                        for version in ['1.3', '1.4', '1.5', '1.6', '1.7']:
                            if version in s2:
                                phred = 'phred64'
    return phred
phredEncoding = get_phred(args.fastqcFile, args.fqName)
print(phredEncoding)
